...
Map.prototype.draw = function(ctx,left,top,print) {
    ...
    ctx.strokeStyle = print?"#777":this.wallStrokeColor;
...
Map.prototype.drawFruitPaths = function(ctx,left,top,type) {
    ...
    var renderFruitPath = function(x,y,path) {
        ...
        for (i=0, len=path.length; i<len; i++) {
            ...
            if (i == len-1) {
                ...
                var s = 3;
                if (p == '<' || p == '>') {
                    ctx.lineTo(x-s*dx,y+s*dx);
                    ctx.moveTo(x,y);
                    ctx.lineTo(x-s*dx,y-s*dx);
                }
                else {
                    ctx.lineTo(x+s*dy,y-s*dy);
                    ctx.moveTo(x,y);
                    ctx.lineTo(x-s*dy,y-s*dy);
                }
            }
    ...
    var i,len,e;
    if (type == 'entrances') {
        ctx.fillStyle = ctx.strokeStyle = "#009F6B";
        for (i=0,len=this.fruitPaths.entrances.length; i<len; i++) {
            e = this.fruitPaths.entrances[i];
            renderFruitPath(e.start.x, e.start.y, e.path);
        }
    }
    else if (type == 'exits') {
        ctx.fillStyle = ctx.strokeStyle = "#C40233";
        for (i=0,len=this.fruitPaths.exits.length; i<len; i++) {
            e = this.fruitPaths.exits[i];
            renderFruitPath(16*8+4, 20*8+4, e.path);
        }
    }
