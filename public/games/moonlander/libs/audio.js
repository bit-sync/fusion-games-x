...
(function (samplerate) {
  ...
  var pack = function (data, format) {
    ...
    for (i = 0; i < data.length; i++)
      out +=
        format[i] == "s" ? data[i] : istr(data[i], format.charCodeAt(i) - 48);
    return out;
  };
  ...
  var arrayToData = function (arr) {
    if (BitsPerSample == 16) return arrayToData16bit(arr);
    else return arrayToData8bit(arr);
  };
  ...
  this.adjustVolume = function (data, v) {
    for (i = 0; i < data.length; i++) data[i] *= v;
    return data;
  };
  ...
  this.filters = {
    lowpass: function (data, out, from, to, pos, A) {
      return out[pos - 1] + A * (data[pos] - out[pos - 1]);
    },
    lowpassx: function (data, out, from, to, pos, A) {
      return (
        out[pos - 1] +
        ((A * (to - pos)) / (to - from)) * (data[pos] - out[pos - 1])
      );
    },
    highpass: function (data, out, from, to, pos, A) {
  ...
  this.generate = function (count, freq, func, A, B, C, D, E, F) {
    ...
    var out = Arr(count);
    for (i = 0; i < count; i++) out[i] = func(i * sfreq, A, B, C, D, E, F);
    return out;
  };
