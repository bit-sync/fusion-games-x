...
(function () {
  ...
  this.getGeneratorNames = function () {
    var names = [];
    for (e in this.Generators) names.push(e);
    return names;
  };
  ...
  this.generate = function (params) {
    ...
    var vibrato_phase_slide =
      1.0 + (pow(params.VibratoFrequencySlide, 3.0) * 3.0) / SampleRate;
    ...
    if (params.ChangeAmount > 0) arpeggiator_mod = 1 + arpeggiator_mod * 10;
    else arpeggiator_mod = 1 - arpeggiator_mod * 0.9;

    // phaser
    var phaser_max = 1024;
    var phaser_mask = 1023;
    var phaser_buffer = new Array(phaser_max);
    for (var _i = 0; _i < phaser_max; _i++) phaser_buffer[_i] = 0;
    var phaser_pos = 0;
    var phaser_offset = pow(params.PhaserOffset, 2.0) * (phaser_max - 4);
    var phaser_offset_slide =
      (pow(params.PhaserSweep, 3.0) * 4000) / SampleRate;
    var phaser_enabled =
      abs(phaser_offset_slide) > 0.00001 || abs(phaser_offset) > 0.00001;

    // lowpass filter
    var filters_enabled =
      params.HPFilterCutoff > 0.001 || params.LPFilterCutoff < 0.999;
    ...
    var lowpass_damping =
      (5.0 / (1.0 + pow(params.LPFilterResonance, 2) * 20)) *
    ...
    for (var i = 0; i < totalSamples; i++) {
      ...
      var _vibrato_phase_mod =
        phase_speed * sin(vibrato_phase) * vibrato_amplitude;
      ...
      if (i > envelope_last) {
        envelope_idx += 1;
        if (envelope_idx < envelopes_len)
          // fault protection
          envelope = envelopes[envelope_idx];
        // the trailing envelope is silence
        else envelope = { from: 0, to: 0, samples: totalSamples };
        envelope_cur = envelope.from;
        envelope_increment =
          (envelope.to - envelope.from) / (envelope.samples + 1);
    ...
    for (var i = 0; i < len; i++) padding[i] = 0;
